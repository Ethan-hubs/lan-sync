...
**一份二进制、一套脚本，同时跑在 Win10 与 Win11 上**——构建目标 `net10.0-windows10.0.17763.0`（API 下限 = Win10 1809），
实际**支持口径按实测**（见 §1 表），
...
| 平台 | 是否支持 | 依据 |
|---|---|---|
| Windows 11（21H2 ~ 26H1，含 Home/Pro） | ✅ 全版本 | .NET 10 官方支持列表 + 真机（Win11 25H2 patch 级已装过） |
| Windows 10（家用/专业版，如 22H2） | ⏳ **待实测**（口径：能装上能跑就给支持） | 原用的 Win10 22H2 机器随 tzgj 线废弃而不可用 ⇒ **当前无 Win10 实测机**，补位方式（VM / 借机 / 口径降级）待项目发起人决策；**补位前不得表述为"已实测"** |
| Windows 10 **LTSC / Enterprise**（1809 起） | ✅ | 与上一行同一二进制；**不作为硬门槛对外承诺** |
| Windows Server 2016 / 2019 / 2022 / 2025 | ✅ | .NET 10 官方支持列表 + 真机（Server 2025 上 `iscc` 编译与装卸测试） |

> **2026-09-20 决策（项目发起人「先按能跑的标准来」）**：支持口径改为**实测口径**——Win11 全版本 / **Windows 10（实测机可跑）** /
> Server 2016+；**不再把「Win10 1809 LTSC」写成硬门槛**（那是构建目标平台的 API 下限，不是对用户的门槛承诺）。
> 构建侧仍钉 `net10.0-windows10.0.17763.0`：它保证二进制**不依赖 17763 之后的 API**，从而在 Win10 上也能跑；
> 家用版 Win10 已 EOL，官方支持列表不列它——我们按"实测能跑"给支持，但不承诺微软级别的安全更新。
...
| **Windows 10** | ~~原 tzgj 开发机~~（**随线废弃，不可用**） | ❌ **无机器**：需先补位再验（ADR-001 §15 第 3 项、§17 连带缺口） |

> **Win10 现状（2026-09-15）**：静态审查（§8）已落地并在 CI 里跑；托盘进程 manifest 的 `longPathAware`
> 已在构建产物 PE 里实测确认。**仍未做的是"真实安装 + 同步"**——原计划的补位机器随 tzgj 线废弃而不可用，**需先决定补位方式**，
