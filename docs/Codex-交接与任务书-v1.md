...
| 项 | 状态 | 证据位置 |
|---|---|---|
| PRD v0.9（实施基线候选 rc） | ✅ 六轮外部评审 P0 已清零 | `/data/lan-sync/局域网同步系统-产品文档-v0.10.md` |
...
- 局域网同步系统-产品文档-v0.10.md 的 §2.4（两层准入）/§2.5（E1–E6）/§2.6（M1 准入 Gate）/§6.1.1（Adapter 职责）/§12.4（上游能力边界）
...
|---|---|---|---|---|
| **1** | Adapter 前四项职责 + events + pause/resume + `/rest/folder/errors` 只读 | `2e965a4` → 变基后 `60393e4`（已推远端） | 首轮：单元 22/22、集成 6/6 + 6/6（**改副本才可构建**）；二次（`4a812fc` 后，**零改动**）：单元 22/22、集成 8/8（外部实例）、8/8（自建·Linux 二进制）、2 passed/6 skipped（无实例）、`dotnet format` 0 → 详见 `docs/独立复核报告-迭代1.md` | ✅ **通过** |
| **1.1** | 复核改进：P1-1 跨平台 TFM、P1-2 哈希按 OS 选择 + 环境变量覆盖 | `829af00`、`4a812fc`（已推远端） | 零改动复核通过；两项标记**已关闭** | ✅ **通过** |
